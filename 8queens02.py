    ...
    for i in range(n):
        for j in range(i+1, n):
            ...
            if queens[i][0] == queens[j][0] or queens[i][1] == queens[j][1]:
                conflict_label.config(text=f"Царица на ({queens[i][0]+1}, {queens[i][1]+1}) е в конфликт с царица на ({queens[j][0]+1}, {queens[j][1]+1})")
...
def add_queen():
    ...
    conflicting_queens_label.config(text="Царици с конфликти: " + str(conflicting_queens))
...
def remove_queen():
    ...
    queens = [(int(text[0]), int(text[1])) for text in board.find_withtag("Q")]
    queens_to_remove = [(r, c) for r, c in queens if r == row and c == col]
    if not queens_to_remove:
        conflict_label.config(text="Няма царица на тази позиция!")
        return

    for queen in queens_to_remove:
        board.delete(board.find_withtag(f"{queen[0]}{queen[1]}"))
    queens = [(int(text[0]), int(text[1])) for text in board.find_withtag("Q")]
    conflicting_queens = queens_conflict(queens)
    conflicting_queens_label.config(text="Царици с конфликти: " + str(conflicting_queens))
    if conflicting_queens:
